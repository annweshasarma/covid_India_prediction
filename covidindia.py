...
num_days_poly = poly.fit_transform(df['Day Number'].as_matrix().reshape(-1,1))
poly_reg = modal_IND.fit(num_days_poly, df['Confirmed'].as_matrix().reshape(-1,1))
...
confirmed_cases = df['Confirmed'].as_matrix()
